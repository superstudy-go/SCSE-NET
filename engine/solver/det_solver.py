...
class DetSolver(BaseSolver):
    ...
    def val(self, ):
        ...
        for name, submodule in module.named_modules():
            if submodule.__class__.__name__ == "ODALStem":
                odal_name = name
                odal_stem = submodule
                break

        # 备用查找方式
        if odal_stem is None:
            for name, submodule in module.named_modules():
                if (
                    hasattr(submodule, "last_orientation_weights")
                    and hasattr(submodule, "odal")
                    and hasattr(submodule, "drfd")
                ):
                    odal_name = name
                    odal_stem = submodule
                    break
        ...
        if odal_stem is None:
            raise RuntimeError("模型中没有找到 ODALStem")

        print("=" * 70)
        print("成功找到 ODALStem")
        print("模块位置：", odal_name)
        print(odal_stem)
        print("=" * 70)
                # ============================================================
        # Representative Target Pattern Candidates for ODAL Responses
        #
        # Goal:
        # Automatically select representative infrared small-target cases:
        # 1. Compact target
        # 2. Elongated target
        # 3. Low-contrast target
        # 4. Cluttered background
        #
        # For each candidate, visualize:
        # Input | Horizontal | Vertical
        # Main-Diagonal | Anti-Diagonal | Max Response
        # ============================================================
        ...
        candidate_dir = Path(self.output_dir) / "odal_target_pattern_candidates"
        ...
        def _response_to_map(resp_chw):
            heat = resp_chw.abs().mean(dim=0).detach().cpu().numpy()
            return heat

        def _normalize_heatmaps_shared(heatmaps):
            all_heat = np.concatenate([h.reshape(-1) for h in heatmaps])
            vmin = np.percentile(all_heat, 2)
            vmax = np.percentile(all_heat, 99.5)

            normed = []
            for h in heatmaps:
                h = np.clip(h, vmin, vmax)
                h = (h - vmin) / (vmax - vmin + 1e-6)
                normed.append(h)
            return normed

        def _parse_box(box, H, W):
            """
            Return absolute xyxy box.
            Compatible with:
            1. normalized cxcywh
            2. absolute xyxy
            3. absolute cxcywh
        ...
        def _make_crop_box(cx, cy, H, W, crop_size=64):
        ...
        def _compute_contrast_and_clutter(gray, box_xyxy):
            """
            gray: numpy [H, W], normalized or raw grayscale
            box_xyxy: absolute xyxy
            """
            H, W = gray.shape
            ...
            # outer region around target
            pad = max(8, int(max(x2 - x1, y2 - y1) * 2))
            ox1 = int(max(0, x1 - pad))
            ox2 = int(min(W, x2 + pad))
            oy1 = int(max(0, y1 - pad))
            oy2 = int(min(H, y2 + pad))

            outer = gray[oy1:oy2, ox1:ox2].copy()

            # remove target area from outer region by mask
            ...
            bg_std = float(background.std() + 1e-6)

            contrast = abs(target_mean - bg_mean) / bg_std
            clutter = bg_std

            return contrast, clutter

        def _plot_single_candidate(record, save_path):
            input_gray = record["input_gray"]
            heat_h = record["heat_h"]
            heat_v = record["heat_v"]
            heat_d = record["heat_d"]
            heat_a = record["heat_a"]

            heat_h, heat_v, heat_d, heat_a = _normalize_heatmaps_shared(
                [heat_h, heat_v, heat_d, heat_a]
            )

            heat_max = np.maximum.reduce([heat_h, heat_v, heat_d, heat_a])

            fig, axes = plt.subplots(2, 3, figsize=(8.5, 5.5))

            axes[0, 0].imshow(input_gray, cmap="gray")
            axes[0, 0].set_title("Input")
            axes[0, 0].axis("off")

            axes[0, 1].imshow(heat_h, cmap="jet")
            axes[0, 1].set_title("Horizontal")
            axes[0, 1].axis("off")

            axes[0, 2].imshow(heat_v, cmap="jet")
            axes[0, 2].set_title("Vertical")
            axes[0, 2].axis("off")

            axes[1, 0].imshow(heat_d, cmap="jet")
            axes[1, 0].set_title("Main Diag.")
            axes[1, 0].axis("off")

            axes[1, 1].imshow(heat_a, cmap="jet")
            axes[1, 1].set_title("Anti Diag.")
            axes[1, 1].axis("off")

            axes[1, 2].imshow(heat_max, cmap="jet")
            axes[1, 2].set_title("Max")
            axes[1, 2].axis("off")
        ...
        print("=" * 70)
        print("Start collecting ODAL target-pattern candidates...")
        print("=" * 70)
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(self.val_dataloader):
                ...
                samples = move_samples_to_device(
                    samples,
                    self.device,
                    non_blocking=True
                )

                images = select_model_input(samples, key="rgb")
                B, C, H, W = images.shape

                feature = stem.conv_init(images)

                h_all = stem.dir_filter.filters[0](feature)
                v_all = stem.dir_filter.filters[1](feature)
                d_all = stem.dir_filter.filters[2](feature)
                a_all = stem.dir_filter.filters[3](feature)

                for i in range(B):
                    target = targets[i] if isinstance(targets, (list, tuple)) else None
                    ...
                    # use the first GT box
                    box_xyxy = _parse_box(boxes[0], H, W)
                    if box_xyxy is None:
                        sample_global_id += 1
                        continue
                    ...
                    area = bw * bh
                    aspect = bw / (bh + 1e-6)
                    elongation = max(aspect, 1.0 / (aspect + 1e-6))

                    cx = (x1 + x2) / 2.0
                    cy = (y1 + y2) / 2.0

                    crop_box = _make_crop_box(
                        cx,
                        cy,
                        H,
                        W,
                    ...
                    yy1, yy2, xx1, xx2 = crop_box

                    image_crop = images[i:i+1, :, yy1:yy2, xx1:xx2]
                    h_crop = h_all[i:i+1, :, yy1:yy2, xx1:xx2]
                    v_crop = v_all[i:i+1, :, yy1:yy2, xx1:xx2]
                    d_crop = d_all[i:i+1, :, yy1:yy2, xx1:xx2]
                    a_crop = a_all[i:i+1, :, yy1:yy2, xx1:xx2]

                    input_gray = _normalize_img(image_crop[0])

                    heat_h = _response_to_map(h_crop[0])
                    heat_v = _response_to_map(v_crop[0])
                    heat_d = _response_to_map(d_crop[0])
                    heat_a = _response_to_map(a_crop[0])

                    # raw gray image for contrast/clutter
                    raw_img = images[i].detach().cpu()
                    raw_gray = raw_img.mean(dim=0).numpy()

                    contrast, clutter = _compute_contrast_and_clutter(
                        raw_gray,
                        box_xyxy
                    )
                ...
                if batch_idx % 20 == 0:
                    print(f"已扫描 batch {batch_idx}, 当前候选样本数: {len(all_records)}")

        if len(all_records) == 0:
            raise RuntimeError("没有收集到候选样本，请检查 targets 中是否包含 boxes。")

        print("=" * 70)
        print("Collected candidate samples:", len(all_records))
        print("=" * 70)

        # ============================================================
        # Select candidates by target pattern
        # ============================================================

        # 1. Compact target: aspect ratio close to 1
        compact_records = sorted(
            all_records,
            key=lambda r: abs(np.log(r["aspect"] + 1e-6))
        )[:top_k]

        # 2. Elongated target: large aspect-ratio deviation
        elongated_records = sorted(
            all_records,
            key=lambda r: r["elongation"],
            reverse=True
        )[:top_k]

        # 3. Low-contrast target: smaller target-background contrast
        low_contrast_records = sorted(
            all_records,
            key=lambda r: r["contrast"]
        )[:top_k]

        # 4. Cluttered background: larger local background standard deviation
        cluttered_records = sorted(
            all_records,
            key=lambda r: r["clutter"],
            reverse=True
        )[:top_k]

        category_map = {
            "compact": compact_records,
            "elongated": elongated_records,
            "low_contrast": low_contrast_records,
            "cluttered_background": cluttered_records,
        }

        # ============================================================
        # Save candidate figures and CSV
        # ============================================================

        csv_path = candidate_dir / "target_pattern_candidates.csv"
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            for category, records in category_map.items():
                for rank, record in enumerate(records, start=1):
                    fig_path = candidate_dir / f"{category}_rank{rank:02d}_sample{record['sample_id']:04d}.png"

                    _plot_single_candidate(
                        record,
                        fig_path
                    )
        ...
        print("=" * 70)
        print("候选图已保存到：", candidate_dir)
        print("候选CSV已保存：", csv_path)
        print("=" * 70)
        print("请优先查看每类 rank01-rank05 的图片，挑出最清楚的一张。")
        print("=" * 70)
        ...
        logger.info(
            GREEN + f"Model Info(fused) {model_info}" + RESET
        )
