...
from engine.deim.hybrid_encoder_improve import TransformerEncoderBlock_improve
...
from engine.extre_module.custom_nn.attention.FSDA import FSDA 
from engine.extre_module.custom_nn.attention.FSDA_Omni import FSDA_Omni
...
from engine.extre_module.custom_nn.module.IDWB_improve import VortexInceptionDWBlock
from engine.extre_module.custom_nn.block.RepHMS import RepHMS
from engine.extre_module.custom_nn.block.MANet import MANet 
from engine.extre_module.custom_nn.block.MANet_improve import MANet_improve
...
from engine.extre_module.custom_nn.transformer.ADHOGSA import ADHOGSA
...
from engine.extre_module.custom_nn.featurefusion.DCGRM import DCGRM
from engine.extre_module.custom_nn.featurefusion.MSCRM import MSCRM 
from engine.extre_module.custom_nn.featurefusion.DCGRM_improve import DCGRM_improve
...
from engine.extre_module.custom_nn.module.AMSI import AMSI 
from engine.extre_module.custom_nn.module.AMSI_improve import AMSI_improve
from engine.extre_module.custom_nn.moe.moe_module import ESMoE  
from engine.extre_module.custom_nn.moe.moe_module_improve import ESMoE_improve
from engine.extre_module.custom_nn.stem.ODALStem import ODALStem
from engine.extre_module.custom_nn.stem.ODALNoDirStem import ODALNoDirStem
...
def parse_module(d, i, f, m, args, ch, nc=None, eval_spatial_size=None):     
    ...
    if m in {StemBlock, HG_Stage}:
    ...
    elif m in {RepNCSPELAN4, CSPLayer, ConvNormLayer_fuse, SCDown, ESMoE, ESMoE_improve}:
        c1, c2 = ch[f], args[0] 
        args = [c1, c2, *args[1:]]  
    elif m in {TransformerEncoderBlock, TransformerEncoderBlock_improve, ADHOGSA}:
        c2 = ch[f]
        args = [c2, *args]
        m_ = m(*args)
    elif m is Concat:  
        c2 = sum(ch[x] for x in f)
    elif m in {ContrastDrivenFeatureAggregation, DownsampleConv, GQL, FSDA, FSDA_Omni}: # attention   
        c2 = ch[f]
        args = [c2, *args]    
    elif m in {PSConv, ADown, Conv, A3Conv, FreqLAWDS, RouterLAWDS, EdgeLAWDS}: # Conv / downsample     
        c1, c2 = ch[f], args[0]
        args = [c1, c2, *args[1:]]
    elif m in {InceptionDWBlock, GlobalBlock, LocalBlock, OPE, VortexInceptionDWBlock, ODALStem, AMSI, AMSI_improve,ODALNoDirStem}: # module
        c1, c2 = ch[f], args[0]   
        args = [c1, c2, *args[1:]]
    ...
    elif m in {RepHMS, MANet, C3_Block, C2f_Block, C3k2_Block, MetaFormer_Block, MetaFormer_Mona, MetaFormer_SEFN, MetaFormer_Mona_SEFN, MANet_improve}: # Block 
        c1, c2 = ch[f], args[0]
        ...
        c2 = ch[f[0]]
    elif m in {CIDAF, WDAF, MFM, MultiScalePCA, MultiScalePCA_Down, DCGRM, DCGRM_improve, MSCRM}: # featurefusion
        c1 = [ch[i] for i in f]
        c2 = args[0]     
        args = [c1, c2, *args[1:]]    
